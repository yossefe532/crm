...
exports.analyticsService = {
    listDailyMetrics: (tenantId, from, to) => client_1.prisma.leadMetricsDaily.findMany({
        where: {
            tenantId,
            metricDate: {
    ...
    getStageDistribution: async (tenantId) => {
    ...
    getConversionRate: async (tenantId) => {
        const total = await client_1.prisma.lead.count({ where: { tenantId, deletedAt: null } });
        const won = await client_1.prisma.lead.count({ where: { tenantId, deletedAt: null, status: "won" } }); // Assuming 'won' is the success status
        return { total, won, rate: total > 0 ? (won / total) * 100 : 0 };
    },
    getSalesPerformance: async (tenantId) => {
        const performance = await client_1.prisma.lead.groupBy({
            by: ["assignedUserId"],
    ...
    getTeamPerformance: async (tenantId) => {
        // Group leads by team
        const performance = await client_1.prisma.lead.groupBy({
            by: ["teamId"],
    ...
    getAvgTimePerStage: async (tenantId) => {
        // This is a heavy query, in production it should be cached or pre-calculated
        const history = await client_1.prisma.leadStateHistory.findMany({
            where: { tenantId },
