...
exports.seedRbacForTenant = void 0;
...
const seedRbacForTenant = async (tenantId) => {
    ...
    const permissionRows = await client_1.prisma.permission.findMany({
    ...
    const tenantRoles = await client_1.prisma.role.findMany({ where: { tenantId, deletedAt: null } });
    ...
    for (const [roleName, codes] of Object.entries(rolePermissions)) {
        ...
        if (!desiredPermissionIds.length)
            continue;
        const existing = await client_1.prisma.rolePermission.findMany({
        ...
        if (toCreate.length) {
            await client_1.prisma.rolePermission.createMany({ data: toCreate });
        }
    }
};
exports.seedRbacForTenant = seedRbacForTenant;
