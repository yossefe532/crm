...
exports.conversationService = {
    ensureOwnerGroup: async (tenantId) => {
        let convo = await client_1.prisma.conversation.findFirst({
            where: { tenantId, type: "owner_group", deletedAt: null }
        });
        if (!convo) {
            convo = await client_1.prisma.conversation.create({
        ...
        const teamLeaderRole = await client_1.prisma.role.findFirst({ where: { tenantId, name: "team_leader", deletedAt: null } });
        const roleIds = [ownerRole?.id, teamLeaderRole?.id].filter(Boolean);
        if (roleIds.length) {
            const users = await client_1.prisma.userRole.findMany({
                where: { tenantId, roleId: { in: roleIds }, revokedAt: null },
                select: { userId: true }
            });
            const userIds = Array.from(new Set(users.map((row) => row.userId)));
            const existing = await client_1.prisma.conversationParticipant.findMany({
            ...
            if (toAdd.length) {
                await client_1.prisma.conversationParticipant.createMany({
            ...
            if (toRemove.length) {
                await client_1.prisma.conversationParticipant.deleteMany({
    ...
    ensureTeamGroup: async (tenantId, teamId) => {
        let convo = await client_1.prisma.conversation.findFirst({
        ...
        const team = await client_1.prisma.team.findFirst({ where: { tenantId, id: teamId, deletedAt: null } });
        const ownerRole = await client_1.prisma.role.findFirst({ where: { tenantId, name: "owner", deletedAt: null } });
        const ownerUsers = ownerRole
            ? await client_1.prisma.userRole.findMany({ where: { tenantId, roleId: ownerRole.id, revokedAt: null }, select: { userId: true } })
            : [];
        const userIds = Array.from(new Set([...(team?.leaderUserId ? [team.leaderUserId] : []), ...members.map((row) => row.userId), ...ownerUsers.map((row) => row.userId)]));
        const existing = await client_1.prisma.conversationParticipant.findMany({
            where: { tenantId, conversationId: convo.id },
            select: { userId: true }
        });
    ...
    ensureDirect: async (tenantId, userId, targetUserId) => {
        const convo = await client_1.prisma.conversation.findFirst({
            where: {
                tenantId,
                type: "direct",
                deletedAt: null,
    ...
    listConversationsForUser: async (tenantId, user) => {
    ...
    listMessages: async (tenantId, conversationId, user, limit = 50) => {
        const canAccess = await client_1.prisma.conversationParticipant.findFirst({
            where: { tenantId, conversationId, userId: user.id }
        });
        if (!canAccess && !user.roles.includes("owner"))
    ...
    sendMessage: async (tenantId, conversationId, user, payload) => {
        const canAccess = await client_1.prisma.conversationParticipant.findFirst({
            where: { tenantId, conversationId, userId: user.id }
        });
        if (!canAccess && !user.roles.includes("owner"))
        ...
        await client_1.prisma.conversation.update({ where: { id: conversationId, tenantId }, data: { updatedAt: new Date() } });
        return message;
    }
};
